...
type AccurateSubmission = { input: AccurateInput; result: AccurateResult };
type QuickSubmission = { input: QuickInput; result: QuickResult };
...
function ResultPlaceholder({
  ...
  mode: "accurate" | "quick";
  onBack: () => void;
  payload: AccurateSubmission | QuickSubmission;
}) {
  return (
    <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)] sm:p-8">
      <div className="space-y-4">
        <p className="text-sm font-semibold text-slate-900">结果页占位（阶段 3 实现）</p>
        <p className="text-sm text-slate-500">当前模式：{mode === "accurate" ? "精确报价" : "快速估算"}</p>
...
export function QuotationShell() {
  ...
  const [accurateSubmission, setAccurateSubmission] = useState<AccurateSubmission | null>(null);
  const [quickSubmission, setQuickSubmission] = useState<QuickSubmission | null>(null);
  ...
  const { getValues, setValue, trigger } = accurateForm;

  useEffect(() => {
    if (hasDoor && !previousHasDoorRef.current) {
      const width = getValues("W");
      const height = getValues("H");

      if (!doorTouchedRef.current.doorWTouched && typeof width === "number") {
        setValue("doorW", width, { shouldDirty: true, shouldValidate: true });
      }

      if (!doorTouchedRef.current.doorHTouched && typeof height === "number") {
        setValue("doorH", height, { shouldDirty: true, shouldValidate: true });
      }
    }

    previousHasDoorRef.current = hasDoor;
    void trigger(["D", "doorW", "doorH", "doorCount", "doorUnitPrice"]);
  }, [getValues, hasDoor, setValue, trigger]);
  ...
  const accurateSubmit = accurateForm.handleSubmit((values) => {
    const result = calculateAccurate(values);
    setAccurateSubmission({ input: values, result });
  });

  const quickSubmit = quickForm.handleSubmit((values) => {
    const result = calculateQuick(values);
    setQuickSubmission({ input: values, result });
  });
      ...
      {activeTab === "accurate" ? (
        <div className="grid gap-6 md:grid-cols-2">
          <section className="rounded-[28px] border border-slate-200 bg-slate-50/70 p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)] sm:p-8">
            <form className="space-y-6" onSubmit={accurateSubmit}>
              <FieldSet title="基础尺寸">
          ...
          {accurateSubmission ? (
            <ResultPlaceholder
              mode="accurate"
              onBack={() => setAccurateSubmission(null)}
              payload={accurateSubmission}
            />
          ) : (
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <section className="rounded-[28px] border border-slate-200 bg-slate-50/70 p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)] sm:p-8">
            <form className="space-y-6" onSubmit={quickSubmit}>
              <div className="grid gap-4">
                <FormField
          ...
          {quickSubmission ? (
            <ResultPlaceholder
              mode="quick"
              onBack={() => setQuickSubmission(null)}
              payload={quickSubmission}
            />
          ) : (
